...
import { validatePropertyAgenda } from '../services/propertyService';
import { Filter } from './filter';

export class ValidateAgendaFilter implements Filter {
  async process(criteria: any): Promise<void> {
    const { propertyId, startDate, endDate } = criteria;
    if (!propertyId || !startDate || !endDate) return;

    try {
      const isAvailable = await validatePropertyAgenda(propertyId, new Date(startDate), new Date(endDate));
      if (!isAvailable) {
